...
export async function POST() {
  try {
    ...
    if ((count ?? 0) > 100) {
      return NextResponse.json({ message: 'Data already seeded', count }, { status: 200 });
    }
    ...
    // Customer external_id → DB id map
    ...
    for (let i = 0; i < transactions.length; i += BATCH_SIZE) {
      ...
      if (txError) {
        console.error('[Seed] TX batch error:', txError.message);
        continue;
      }

      txInserted += insertedTxs?.length ?? 0;

      // Calculate risk scores for each transaction in batch
      for (let j = 0; j < batch.length; j++) {
        ...
        if (riskScore) {
          ...
          // Create risk cases for HIGH + CRITICAL transactions
          if (engineResult.level === 'HIGH' || engineResult.level === 'CRITICAL') {
            caseCounter++;
            const year = new Date().getFullYear();
            const caseNumber = `CASE-${year}-${String(caseCounter).padStart(4, '0')}`;
